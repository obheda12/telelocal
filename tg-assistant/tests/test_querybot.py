...
class TestScaffoldCommandHandlers:
    ...
    async def test_summary_command_uses_time_window(self):
        ...
        llm_kwargs = mock_llm.query.call_args.kwargs
        assert llm_kwargs["context_max_chars"] == 200000  # always max for detailed
        assert llm_kwargs["max_tokens_override"] == 11000  # min(16k, 8k+3k)
        assert llm_kwargs["min_messages_per_group"] == 2
...
class TestMentionsScalingParams:
    def test_quick_1d(self):
        ...
        assert limit == 500      # all mentions in timeframe
        assert ctx == 98000      # min(200000, 80000+18000)
...
class TestChatDeepLink:
    ...
    def test_regular_group_returns_tg_scheme(self):
        assert _chat_deep_link(-4578576379, 500) == "tg://openmessage?chat_id=4578576379&msgid=500"

    def test_small_regular_group(self):
        assert _chat_deep_link(-123456, 42) == "tg://openmessage?chat_id=123456&msgid=42"

    def test_private_chat_returns_none(self):
        assert _chat_deep_link(999999, 1) is None

    def test_boundary_value(self):
        assert _chat_deep_link(-1_000_000_000_000, 1) == "tg://openmessage?chat_id=1000000000000&msgid=1"
...
class TestBuildChatLinkMap:
    ...
    def test_regular_group_uses_tg_scheme(self):
        ...
        link_map = _build_chat_link_map(results)
        assert link_map["small group"] == "tg://openmessage?chat_id=123456&msgid=1"
